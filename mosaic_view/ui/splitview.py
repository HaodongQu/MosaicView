...
class SplitView(QtWidgets.QFrame):
    ...
    def __init__(self, pixmap_main_topleft=None, filename_main_topleft=None, name=None, 
        ...
        self.overlay_image_count = sum([self.pixmap_topright_exists, self.pixmap_bottomright_exists, self.pixmap_bottomleft_exists])
        self.overlay_layout_mode = self.determine_overlay_layout_mode()
        ...
        self._view_layoutdriving_topleft.setMaximumWidth(self.width() // 2)
        self._view_layoutdriving_topleft.setMaximumHeight(self.height() // 2)
        ...
        self.split_boundary_vertical_top = QtWidgets.QFrame(self)
        self.split_boundary_vertical_bottom = QtWidgets.QFrame(self)
        self.split_boundary_horizontal_left = QtWidgets.QFrame(self)
        self.split_boundary_horizontal_right = QtWidgets.QFrame(self)
        self.split_boundary_lines = [
            self.split_boundary_vertical_top,
            self.split_boundary_vertical_bottom,
            self.split_boundary_horizontal_left,
            self.split_boundary_horizontal_right,
        ]
        self.init_split_boundary_overlay()

        # Set layout
        self._layout.addWidget(self._view_main_topleft, 0, 0, 2, 2)
        self.configure_overlay_views_for_mode()
    ...
    def determine_overlay_layout_mode(self):
        """Determine how overlay images should be arranged."""
        return determine_overlay_layout_mode(
            self.pixmap_topright_exists,
            self.pixmap_bottomleft_exists,
            self.pixmap_bottomright_exists,
        )

    def configure_overlay_views_for_mode(self):
        """Arrange overlay views according to the selected layout mode."""
        for widget in [
            self._view_layoutdriving_topleft,
            self._view_topright,
            self._view_bottomleft,
            self._view_bottomright,
        ]:
            self._layout.removeWidget(widget)

        self._view_topright.hide()
        self._view_bottomleft.hide()
        self._view_bottomright.hide()

        if self.overlay_layout_mode == "left-right":
            self._layout.addWidget(self._view_layoutdriving_topleft, 0, 0, 2, 1)
            self.left_right_overlay_view().show()
            self._layout.addWidget(self.left_right_overlay_view(), 0, 1, 2, 1)
        elif self.overlay_layout_mode == "top-bottom":
            self._layout.addWidget(self._view_layoutdriving_topleft, 0, 0, 1, 2)
            self.top_bottom_overlay_view().show()
            self._layout.addWidget(self.top_bottom_overlay_view(), 1, 0, 1, 2)
        else:
            self._layout.addWidget(self._view_layoutdriving_topleft, 0, 0)
            self._view_topright.show()
            self._view_bottomleft.show()
            self._view_bottomright.show()
    ...
    def left_right_overlay_view(self):
        """Return the overlay view used for left-right mode."""
        if self.pixmap_topright_exists:
            return self._view_topright
        if self.pixmap_bottomright_exists:
            return self._view_bottomright
        return self._view_topright

    def left_right_overlay_scale(self):
        """Return zoom-adjust factor for the overlay view used in left-right mode."""
        if self.pixmap_topright_exists:
            return self._topright_zoom_adjust
        if self.pixmap_bottomright_exists:
            return self._bottomright_zoom_adjust
        return 1.0

    def top_bottom_overlay_view(self):
        """Return the overlay view used for top-bottom mode."""
        if self.pixmap_bottomleft_exists:
            return self._view_bottomleft
        if self.pixmap_bottomright_exists:
            return self._view_bottomright
        return self._view_bottomleft

    def top_bottom_overlay_scale(self):
        """Return zoom-adjust factor for the overlay view used in top-bottom mode."""
        if self.pixmap_bottomleft_exists:
            return self._bottomleft_zoom_adjust
        if self.pixmap_bottomright_exists:
            return self._bottomright_zoom_adjust
        return 1.0

    def normalized_split_widget_coordinates(self, point_of_mouse_on_widget):
        """Normalize split coordinates for the active overlay mode."""
        point_of_mouse_on_widget_x = max(int(point_of_mouse_on_widget.x()), 0)
        point_of_mouse_on_widget_y = max(int(point_of_mouse_on_widget.y()), 0)

        if self.overlay_layout_mode == "left-right":
            point_of_mouse_on_widget_y = self.height() // 2
        elif self.overlay_layout_mode == "top-bottom":
            point_of_mouse_on_widget_x = self.width() // 2

        return point_of_mouse_on_widget_x, point_of_mouse_on_widget_y
    ...
    def normalize_image_path(self, filepath):
        """Return a normalized absolute path for a loaded image file."""
        if not filepath:
            return None
        file_info = QtCore.QFileInfo(filepath)
        return file_info.canonicalFilePath() or file_info.absoluteFilePath()

    def set_image_paths(self, filename_main_topleft=None, filename_topright=None, filename_bottomleft=None, filename_bottomright=None):
        """Store filepaths for all image slots used by the active view."""
        self.filename_main_topleft = self.normalize_image_path(filename_main_topleft)
    ...
    def init_split_boundary_overlay(self):
        """Create overlay lines that sit above the image instead of affecting layout."""
        boundary_stylesheet = "background-color: rgba(255, 181, 0, 223); border: 0px;"
        for line in self.split_boundary_lines:
            line.setAttribute(QtCore.Qt.WA_TransparentForMouseEvents)
            line.setStyleSheet(boundary_stylesheet)
            line.hide()
            line.raise_()

    def refresh_split_boundary_overlay(self, split_x, split_y):
        """Position split boundary lines above the image without consuming layout space."""
        line_thickness = 2
        split_x = max(0, min(int(split_x), self.width()))
        split_y = max(0, min(int(split_y), self.height()))
        vertical_x = max(0, split_x - line_thickness // 2)
        horizontal_y = max(0, split_y - line_thickness // 2)

        if self.overlay_layout_mode == "left-right":
            if self.overlay_image_count >= 1 and split_x < self.width():
                self.split_boundary_vertical_top.setGeometry(vertical_x, 0, line_thickness, self.height())
                self.split_boundary_vertical_top.show()
            else:
                self.split_boundary_vertical_top.hide()
            self.split_boundary_vertical_bottom.hide()
            self.split_boundary_horizontal_left.hide()
            self.split_boundary_horizontal_right.hide()
            for line in self.split_boundary_lines:
                line.raise_()
            return

        if self.overlay_layout_mode == "top-bottom":
            if self.overlay_image_count >= 1 and split_y < self.height():
                self.split_boundary_horizontal_left.setGeometry(0, horizontal_y, self.width(), line_thickness)
                self.split_boundary_horizontal_left.show()
            else:
                self.split_boundary_horizontal_left.hide()
            self.split_boundary_vertical_top.hide()
            self.split_boundary_vertical_bottom.hide()
        ...
        vertical_full_height = (
            (self.pixmap_topright_exists and self.pixmap_bottomright_exists) or
            (self.pixmap_topright_exists and not self.pixmap_bottomleft_exists and not self.pixmap_bottomright_exists)
        )
        horizontal_full_width = (
            (self.pixmap_bottomleft_exists and self.pixmap_bottomright_exists) or
            (self.pixmap_bottomleft_exists and not self.pixmap_topright_exists and not self.pixmap_bottomright_exists)
        )

        show_vertical_top = self.pixmap_topright_exists
        show_vertical_bottom = self.pixmap_bottomright_exists
        if vertical_full_height:
            show_vertical_top = True
            show_vertical_bottom = True

        show_horizontal_left = self.pixmap_bottomleft_exists
        show_horizontal_right = self.pixmap_bottomright_exists
        if horizontal_full_width:
            show_horizontal_left = True
            show_horizontal_right = True

        if show_vertical_top and split_y > 0:
            self.split_boundary_vertical_top.setGeometry(vertical_x, 0, line_thickness, split_y)
            self.split_boundary_vertical_top.show()
        else:
            self.split_boundary_vertical_top.hide()

        if show_vertical_bottom and split_y < self.height():
            self.split_boundary_vertical_bottom.setGeometry(vertical_x, split_y, line_thickness, self.height() - split_y)
            self.split_boundary_vertical_bottom.show()
        else:
            self.split_boundary_vertical_bottom.hide()

        if show_horizontal_left and split_x > 0:
            self.split_boundary_horizontal_left.setGeometry(0, horizontal_y, split_x, line_thickness)
            self.split_boundary_horizontal_left.show()
        else:
            self.split_boundary_horizontal_left.hide()

        if show_horizontal_right and split_x < self.width():
            self.split_boundary_horizontal_right.setGeometry(split_x, horizontal_y, self.width() - split_x, line_thickness)
            self.split_boundary_horizontal_right.show()
        else:
            self.split_boundary_horizontal_right.hide()
    ...
    def set_split(self, x_percent=0.5, y_percent=0.5, ignore_lock=False, percent_of_visible=False):
        ...
        This is needed to reposition the split programmatically without mouse movement from the user.
        ...
        Args:
            ...
            ignore_lock (bool): Legacy argument retained for call compatibility.
    ...
    def update_split(self, pos = None, pos_is_global=False, ignore_lock=False): 
        ...
        point_of_mouse_on_widget_x, point_of_mouse_on_widget_y = self.normalized_split_widget_coordinates(point_of_mouse_on_widget)

        self.last_updated_point_of_split_on_scene_main = self._view_main_topleft.mapToScene(point_of_mouse_on_widget_x, point_of_mouse_on_widget_y)
        
        point_of_bottom_right_on_widget = QtCore.QPoint(self.width(), self.height())
        
        point_of_widget_origin_on_scene_main = self._view_main_topleft.mapToScene(0,0)

        point_of_split_on_scene_main = self._view_main_topleft.mapToScene(point_of_mouse_on_widget_x, point_of_mouse_on_widget_y)
        ...
        if self.overlay_layout_mode == "left-right":
            self._view_layoutdriving_topleft.setMaximumWidth(point_of_mouse_on_widget_x)
            self._view_layoutdriving_topleft.setMaximumHeight(self.height())
        elif self.overlay_layout_mode == "top-bottom":
            self._view_layoutdriving_topleft.setMaximumWidth(self.width())
            self._view_layoutdriving_topleft.setMaximumHeight(point_of_mouse_on_widget_y)
        else:
            self._view_layoutdriving_topleft.setMaximumWidth(point_of_mouse_on_widget_x)
            self._view_layoutdriving_topleft.setMaximumHeight(point_of_mouse_on_widget_y)
        self.refresh_split_boundary_overlay(point_of_mouse_on_widget_x, point_of_mouse_on_widget_y)
        
        render_buffer = 100 # Needed to prevent slight pixel offset of the sliding overlays when zoomed-out below ~0.5x

        if self.overlay_layout_mode == "left-right":
            scale_right = 1 / self.left_right_overlay_scale()
            top_left_of_scene_right = QtCore.QPointF(point_of_split_on_scene_main.x() * scale_right, point_of_widget_origin_on_scene_main.y() * scale_right)
            bottom_right_of_scene_right = QtCore.QPointF(point_of_bottom_right_on_scene_main.x() * scale_right + render_buffer, point_of_bottom_right_on_scene_main.y() * scale_right + render_buffer)
            rect_of_scene_right = QtCore.QRectF(top_left_of_scene_right, bottom_right_of_scene_right)
            right_overlay_view = self.left_right_overlay_view()
            right_overlay_view.setSceneRect(rect_of_scene_right)
            right_overlay_view.centerOn(top_left_of_scene_right)
            for overlay_view in [self._view_topright, self._view_bottomleft, self._view_bottomright]:
                if overlay_view is not right_overlay_view:
                    overlay_view.setSceneRect(QtCore.QRectF())
        elif self.overlay_layout_mode == "top-bottom":
            scale_bottom = 1 / self.top_bottom_overlay_scale()
            top_left_of_scene_bottom = QtCore.QPointF(point_of_widget_origin_on_scene_main.x() * scale_bottom, point_of_split_on_scene_main.y() * scale_bottom)
            bottom_right_of_scene_bottom = QtCore.QPointF(point_of_bottom_right_on_scene_main.x() * scale_bottom + render_buffer, point_of_bottom_right_on_scene_main.y() * scale_bottom + render_buffer)
            rect_of_scene_bottom = QtCore.QRectF(top_left_of_scene_bottom, bottom_right_of_scene_bottom)
            bottom_overlay_view = self.top_bottom_overlay_view()
            bottom_overlay_view.setSceneRect(rect_of_scene_bottom)
            bottom_overlay_view.centerOn(top_left_of_scene_bottom)
            for overlay_view in [self._view_topright, self._view_bottomleft, self._view_bottomright]:
                if overlay_view is not bottom_overlay_view:
                    overlay_view.setSceneRect(QtCore.QRectF())
        else:
    ...
    def on_right_click_ruler(self, scene_pos, relative_origin_position, unit="px", px_per_unit=1.0, update_px_per_unit_on_existing=False):
        ...
        pos_p1 = self._view_main_topleft.mapToScene(int(widget_width * placement_factor), int(widget_height * placement_factor))
        pos_p2 = self._view_main_topleft.mapToScene(int(widget_width * (1 - placement_factor)), int(widget_height * (1 - placement_factor)))
    ...
    def update_mouse_rect(self, pos = None):
        ...
        mouse_rect_pos_origin = self._view_main_topleft.mapToScene(int(point_of_mouse_on_widget.x()), int(point_of_mouse_on_widget.y()))
    ...
    def pixmap_topright(self, pixmap):
        self._pixmapItem_topright.setPixmap(pixmap)
    ...
    def pixmap_bottomright(self, pixmap):
        self._pixmapItem_bottomright.setPixmap(pixmap)
    ...
    def pixmap_bottomleft(self, pixmap):
        self._pixmapItem_bottomleft.setPixmap(pixmap)
    
    def resizeEvent(self, event):
        """Override resize event of frame to ensure scene is also resized."""
        self.resize_scene()
        if self.last_updated_point_of_split_on_scene_main:
            self.refresh_split_based_on_last_updated_point_of_split_on_scene_main()
        else:
            self.refresh_split_boundary_overlay(self.width() // 2, self.height() // 2)
